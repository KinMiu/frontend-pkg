...
const EventList: React.FC = () => {
  ...
  const eventsForRole = isOperator && operatorSatminkal
    ? events.filter((e) => (e.satminkal || '').trim() === operatorSatminkal)
    : events;

  const filteredEvents = eventsForRole.filter((event) =>
    ...
    (event.satminkal || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (event.program || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
            ...
            >
              ...
              <div className="relative h-48 w-full">
                ...
                <div className="absolute top-2 left-2 flex flex-wrap gap-2">
                  ...
                  {event.program && (
                    <span className="bg-emerald-600 text-white text-xs font-semibold px-2 py-1 rounded-md shadow">
                      {event.program}
                    </span>
                  )}
